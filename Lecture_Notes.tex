...
$$\boxed{\boxed{
\int_\mathcal M \e^{\frac i\hbar S} \mathcal{O} \d^N x := \frac{``\mathrm{Vol}(G)"}{(2\pi\hbar)^k}\left(\frac\hbar i\right)^k \int_\mathcal M \int \mathfrak g \int_{\ppi(\mathfrak g^*\oplus\mathfrak g)} \d^Nx\d^k\lambda(\D c\D\bar c)^k \e^{\frac i\hbar S_{\mathrm{FP}}(x,\lambda,c,\bar c)}\mathcal{O}(x)
}}$$
où:
$$\boxed{
